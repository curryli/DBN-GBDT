...
class DBN(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, n_ins=24,               ###################################################
                 hidden_layers_sizes=[24,20,18,16], n_outs=2):               
...
################################################################################################
def test_DBN(finetune_lr=0.2, pretraining_epochs=30,
             pretrain_lr=0.1, k=1, training_epochs=200,
             batch_size = 1):
################################################################################################
    """
...
###########################################change feature size : n_ins and label size: n_outs################################################################
    dbn = DBN(numpy_rng=numpy_rng, n_ins=24,
              hidden_layers_sizes=[24,20,18,16],
              n_outs=2)
...
###############################################################################################################################################
    ...
    for i in dbn.params:
        print i.eval().shape
        RbmParamList.append(i.eval())
        
    with open('RbmParamList.pkl', 'w') as f:
                        pickle.dump(RbmParamList, f)


def GBDTclassify(X, y):
...
def testpredict(batch_size = 1):
    RbmParamList = pickle.load(open('RbmParamList.pkl'))
    X = numpy.loadtxt("german2.csv",  delimiter = "," , usecols=(range(24)) , dtype=theano.config.floatX)
    W1 = RbmParamList[0]
    b1 = RbmParamList[1]
    W2 = RbmParamList[2]
    b2 = RbmParamList[3]
...
#    print a2.shape
    ...
    y = numpy.loadtxt("german2.csv",  delimiter = "," , usecols=(24,) , dtype=int)

    clf = GBDTclassify(z1, y)
 
    print m.classification_report(y, clf.predict(z1))
